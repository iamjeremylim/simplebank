...
import (
	...
	mockdb "github.com/iamjeremylim/simplebank/db/mock"
	db "github.com/iamjeremylim/simplebank/db/sqlc"
	"github.com/iamjeremylim/simplebank/token"
...
func TestGetAccountAPI(t *testing.T) {
	...
	testCases := []struct{
		...
		},
		{
			...
			buildStubs: func(store *mockdb.MockStore) {
				store.EXPECT().
				GetAccount(gomock.Any(), gomock.Any()). // I expect GetAccount fn to be called with any ctx and a specific ID arg
				Times(0)
			},
			checkResponse: func(t *testing.T, recorder *httptest.ResponseRecorder) {
	...
	for i := range testCases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// send our API req through the server router, and record response in the recorder
	...
	testCases := []struct{
	...
	}{
		{
			...
			buildStubs: func(store *mockdb.MockStore) {
				...
				store.EXPECT().
				CreateAccount(gomock.Any(), gomock.Eq(arg)). // I expect GetAccount fn to be called with any ctx and a specific ID arg
				Times(1).
				Return(account, nil)
			},
			checkResponse: func(recorder *httptest.ResponseRecorder) {
	...
	testCases := []struct{
	...
	}{
		{
			...
			buildStubs: func(store *mockdb.MockStore) {
				...
				store.EXPECT().
				ListAccounts(gomock.Any(), gomock.Eq(arg)). // I expect GetAccount fn to be called with any ctx and a specific ID arg
				Times(1).
				Return(accounts, nil)
			},
			checkResponse: func(recorder *httptest.ResponseRecorder) {
